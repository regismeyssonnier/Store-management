<?php
	...
	if(isset($_POST['ajax_d']))
	{
		...
		if(($choix_suivi == 'division') || ($choix_suivi == 'division_service'))
		{
			...
			{
				$sql_diag     = "SELECT MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), C.REFERENCE, DESIGNATION, SUM( QTE_DEMANDER ) , PRIX_CONSO, DATE_DEMANDE, 10
								 ...
								 GROUP BY MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), C.REFERENCE, PRIX_CONSO
								 UNION 
								 SELECT MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), C.REFERENCE, DESIGNATION, SUM( QTE_DEMANDER ) , PRIX_CONSO, DATE_DEMANDE, 20
								 FROM CONSOMMABLE AS C
								 ...
								 WHERE DATE_DEMANDE BETWEEN '" .$_POST['date1'] ."' AND '" .$_POST['date2'] ."'
								 GROUP BY MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), C.REFERENCE, PRIX_CONSO
								 ORDER BY MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), REFERENCE, PRIX_CONSO;";
		...
		else if($choix_suivi == 'service')
		{
			$sql_diag = "SELECT MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), C.REFERENCE, DESIGNATION, SUM( QTE_DEMANDER ) , PRIX_CONSO, DATE_DEMANDE, 10
						 ...
						 GROUP BY MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), REFERENCE, PRIX_CONSO
						 ORDER BY MONTH(DATE_DEMANDE), YEAR(DATE_DEMANDE), REFERENCE, PRIX_CONSO;";
	...
	{
		if(($choix_suivi == 'division') || ($choix_suivi == 'division_service'))
			echo 'onmouseover="Souligne(\'pg_prec\');" onmouseout="DeSouligne(\'pg_prec\');" onclick="Diagramme_pagination_division_ev(' .$p_prec .", '" .$date_1 ."', '" .$date_2 ."', '" .$division ."', '" .$choix_suivi .'\');"';
		else if($choix_suivi == 'service')
			echo 'onmouseover="Souligne(\'pg_prec\');" onmouseout="DeSouligne(\'pg_prec\');" onclick="Diagramme_pagination_service_ev(' .$p_prec .", '" .$date_1 ."', '" .$date_2 ."', '" .$service .'\');"';
	...
	if($page > 1)
		echo "<< page prec";
	echo '</td>';
	echo '<td class="cell_pied_tabm">Evolution de la demande</td>';
	echo '<td class="cell_pied_tabc" id="pg_suiv"';
	$p_suiv = $page + 1;
	if($page < $max_page)
	{
		if(($choix_suivi == 'division') || ($choix_suivi == 'division_service'))
			echo 'onmouseover="Souligne(\'pg_suiv\');" onmouseout="DeSouligne(\'pg_suiv\');" onclick="Diagramme_pagination_division_ev(' .$p_suiv .", '" .$date_1 ."', '" .$date_2 ."', '" .$division ."', '" .$choix_suivi .'\');"';
		else if($choix_suivi == 'service')
			echo 'onmouseover="Souligne(\'pg_suiv\');" onmouseout="DeSouligne(\'pg_suiv\');" onclick="Diagramme_pagination_service_ev(' .$p_suiv .", '" .$date_1 ."', '" .$date_2 ."', '" .$service .'\');"';
	...
	if(($choix_suivi == 'division') || ($choix_suivi == 'division_service'))
		echo '<input type="button" name="valider" value="Valider" onclick="Diagramme_changer_type_div_ev(' .$page .", '" .$date_1 ."', '" .$date_2 ."', '" .$division ."', '" .$choix_suivi .'\');" />';
	else if($choix_suivi == 'service')
		echo '<input type="button" name="valider" value="Valider" onclick="Diagramme_changer_type_div_ev(' .$page .", '" .$date_1 ."', '" .$date_2 ."', '" .$service .'\');" />';
	...
	//$t_baton = array(10000,21000,21000,32000,25000,16000,17000,28000,32000,15000,32000,15000);
	...
	{
		if($_POST['division'] == 'Toute')
			$d->set_titre('Evolution de la demande du Rectorat du ' .$dt1 .' au ' .$dt2);
		else
			$d->set_titre('Evolution de la demande du ' .$_POST['division'] .' du ' .$dt1 .' au ' .$dt2);
		
	}
	else
		$d->set_titre('Evolution de la demande du ' .$_POST['service'] .' du ' .$dt1 .' au ' .$dt2);
	...
	{
		if(($choix_suivi == 'division') || ($choix_suivi == 'division_service'))
			echo '<area shape="rect" coords="' .$t[$i]->point1->x  ."," .$t[$i]->point1->y ."," .$t[$i]->point2->x ."," .$t[$i]->point2->y .'" href="javascript:Camembert_ev_division(\'' .$date_1 ."', '" .$date_2 ."', '" .$division ."', '" .$t[$i]->valeur ."', '" .$choix_suivi .'\');" alt="' .$t[$i]->nom .'"/>';
		else if($choix_suivi == 'service')
			echo '<area shape="rect" coords="' .$t[$i]->point1->x  ."," .$t[$i]->point1->y ."," .$t[$i]->point2->x ."," .$t[$i]->point2->y .'" href="javascript:Camembert_ev_service(\'' .$date_1 ."', '" .$date_2 ."', '" .$service ."', '" .$t[$i]->valeur .'\');" alt="' .$t[$i]->nom .'"/>';
