	...
	class Camembert
	{
		...
		public function __construct($n, $f, $l, $h){
			...
			if($n != null)
				...
				$this->nom_fichier = 'camembert/camembert' .date("H_i_s") .rand() .'.png';
			$this->freq = $f;
			...
			if($this->indicateur)
			{
				$this->margex = $this->taille_indicateur + 15;
		...
		public function set_position_titre($x, $y){
		...
		public function Dessine(){
			...
			$tot_deg = 0;$tab_deg = array(0);$tot_d = 0;
			for($i = 0;$i < $this->max_freq;$i++)
			{
				...
				//echo $tot_deg ." " .($tot_deg+$degre) ." " .$degre .'<br/>';
				
				if($degre > 1)
					imagefilledarc($this->id_image, $this->centre_x + ($x * $ec) , $this->centre_y + ($y * $ec), $this->largeur_cam, $this->hauteur_cam, $tot_deg, $tot_d, $this->palette[255/($this->max_freq+1)*($i+1)] , IMG_ARC_NOFILL&IMG_ARC_EDGED);
				else
				{
					/*if($this->etat == 3)
					{
						...
						{*/
							$ad = $tot_deg - 1;
							$af = $tot_deg + 1;
							imagefilledarc($this->id_image, $this->centre_x + ($x * $ec) , $this->centre_y + ($y * $ec), $this->largeur_cam, $this->hauteur_cam, $ad, $af, $this->palette[255/($this->max_freq+1)*($i+1)] , IMG_ARC_NOFILL&IMG_ARC_EDGED);
			...
			if($this->titre != '')
			{
				if(($this->pos_x_titre == '') && ($this->pos_y_titre == ''))
				{
				...
				imagestring($this->id_image, 5, $x, $y, $this->titre, $this->palette['noir']);	
