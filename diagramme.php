<?php
	...
	class Diagramme
	{
		...
		private $max_grille;
		private $titre_axe_y;
		private $ajouter_titre_axe_y_lib;
		private $lib_dessous_freq;
		private $tab_valeur_baton;
		private $max_val_baton;
		...
		public function __construct($n, $f, $l, $h){
			...
			if($n != null)
				...
				$this->nom_fichier = 'diagramme/diag_' .date("H_i_s") .rand() .'.png';
			$this->freq = $f;
			...
			$this->max_grille = '';
			$this->titre_axe_y = '';
			$this->ajouter_titre_axe_y_lib = false;
			$this->lib_dessous_freq = false;
			$this->tab_valeur_baton = '';
			$this->max_val_baton = 0;
		...
		public function set_titre_axe_y($t){
		
			$this->titre_axe_y = $t;
		
		}
		
		public function set_ajouter_titre_axe_y_lib($t){
		
			$this->ajouter_titre_axe_y_lib = $t;
		
		}
		
		public function set_lib_dessous_freq($l){
		
			$this->lib_dessous_freq = $l;
		
		}
		
		public function set_tab_valeur_baton($t){
		
			$this->tab_valeur_baton = $t;
			$this->max_val_baton = count($t);
		...
		public function Dessine(){
			...
			if($this->titre_axe_y != '');
			{
				$x = ( $this->marge_gauche - (strlen($this->titre_axe_y)*6) ) / 2;
				$y = $this->marge_hb - 20;
				imagestring($this->id_image, 2, $x, $y, $this->titre_axe_y, $this->palette['noir']);	
			}
			...
			for($i = 0;($i < $this->max_freq) && ($i < round($this->nb_freq_dess));$i++)
			{
				...
				if( ($y1 - $y1_b) < 1)
					$p1 = new Point(round($x1_b), round($y1_b-1));
				else
				...
				if(($this->tab_valeur_baton != '') && ($i < $this->max_val_baton))
					$mp->valeur = $this->tab_valeur_baton[$i];
			...
			if($this->pos_lib == 1)
			{
				...
				$y = 0;
				if($this->lib_dessous_freq)
					$y = ($this->hauteur - ($this->max_freq * 15 * 2)) / 2;
				else
				...
				for($i = 0;$i < $this->max_freq;$i++)
				{
					...
					$str = $i+1 ." - " .$this->freq[$i];
					if($this->ajouter_titre_axe_y_lib)
						$str .= " " .$this->titre_axe_y;
					if(($this->tab_lib != '') && ($i < $this->max_lib) && !$this->lib_dessous_freq)
						$str .= ' ' .$this->tab_lib[$i];
					imagestring($this->id_image, 2, $x+25, $y, $str, $this->palette['noir']);	
										
					if(($this->lib_dessous_freq) && ($this->tab_lib != '') && ($i < $this->max_lib))
					{
						imagestring($this->id_image, 2, $x+25, $y+15, $this->tab_lib[$i], $this->palette['noir']);
						$y += 30;
					}
					else
						$y += 15;
	...
	$d = new Diagramme('diagramme1.png', $f, 700, 450);
